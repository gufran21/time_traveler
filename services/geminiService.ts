...
function getFallbackPrompt(decade: string): string {
    ...
    // Use a recent year to determine if it's "future".
    const isFuture = decadeAsNumber > new Date().getFullYear();

    if (isFuture) {
        return `Create a futuristic photograph of the person in this image as if they were living in the ${decade}. The image should show futuristic fashion, hairstyles, and a futuristic atmosphere. Ensure the final image is a clear photograph that looks like it could be from that future era.`;
    }
