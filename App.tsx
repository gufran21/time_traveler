...
const DECADES = ['1950s', '1960s', '1970s', '1980s', '1990s', '2000s', '2050s', '2070s', '2100s'];

// Pre-defined positions for a scattered look on desktop
const POSITIONS = [
    { top: '5%', left: '10%', rotate: -8 },   // 1950s
    { top: '15%', left: '60%', rotate: 5 },    // 1960s
    { top: '45%', left: '5%', rotate: 3 },    // 1970s
    { top: '2%', left: '35%', rotate: 10 },   // 1980s
    { top: '40%', left: '70%', rotate: -12 },  // 1990s
    { top: '50%', left: '38%', rotate: -3 },   // 2000s
    { top: '25%', left: '80%', rotate: 8 },    // 2050s
    { top: '55%', left: '20%', rotate: -5 },   // 2070s
    { top: '5%', left: '85%', rotate: 15 },    // 2100s
];
...
const GHOST_POLAROIDS_CONFIG = [
  ...
  { initial: { x: "200%", y: "50%", rotate: 35 }, transition: { delay: 0.9 } },
  { initial: { x: "50%", y: "-250%", rotate: 5 }, transition: { delay: 0.1 } },
];
...
const getPromptForDecade = (decade: string): string => {
    const decadeAsNumber = parseInt(decade.substring(0, 4));
    const isFuture = decadeAsNumber > new Date().getFullYear();
    if (isFuture) {
        return `Imagine the person in this photo living in the ${decade}. Create a futuristic, photorealistic image showing what they might look like. Consider futuristic fashion, hairstyles, technology, and the overall aesthetic of that era.`;
    }
...
function App() {
    ...
    const handleImageUpload = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onloadend = () => {
                ...
                setSelectedDecades(DECADES); // Reset selection
            };
            reader.readAsDataURL(file);
        }
    };

    const handleDecadeSelectionChange = (decade: string) => {
    ...
    const handleDownloadIndividualImage = (decade: string) => {
        ...
        if (image?.status === 'done' && image.url) {
            ...
            link.href = image.url;
            link.download = `time-travel-${decade}.jpg`;
    ...
    const handleDownloadAlbum = async () => {
        ...
        try {
            ...
            link.download = 'time-travel-album.jpg';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
                <div className="text-center mb-10">
                    <h1 className="text-6xl md:text-8xl font-caveat font-bold text-neutral-100">Time Travel</h1>
                ...
                {appState === 'image-uploaded' && uploadedImage && (
                    <div className="flex flex-col items-center gap-6">
                        ...
                        <div className="w-full max-w-md text-center mt-2">
                            <h2 className="font-permanent-marker text-2xl text-neutral-200 mb-4">Select Decades</h2>
                            <div className="grid grid-cols-3 gap-3 mb-4">
                                {DECADES.map((decade) => (
                                    <button
                                        key={decade}
                                        onClick={() => handleDecadeSelectionChange(decade)}
                                        aria-pressed={selectedDecades.includes(decade)}
                                        className={`font-permanent-marker text-lg p-3 rounded-sm border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-black focus:ring-yellow-400 ${
                                            selectedDecades.includes(decade)
                                                ? 'bg-yellow-400 border-yellow-400 text-black shadow-[1px_1px_0px_1px_rgba(0,0,0,0.2)]'
                            ...
                            <button 
                                onClick={toggleSelectAllDecades}
                                className="text-sm text-neutral-400 hover:text-yellow-400 underline transition-colors"
                            >
                                {selectedDecades.length === DECADES.length ? 'Deselect All' : 'Select All'}
                            </button>
                        </div>
                         ...
                         <div className="flex items-center gap-4 mt-2">
                            ...
                            <button 
                                ...
                                className={`${primaryButtonClasses} disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:-rotate-0 disabled:hover:bg-yellow-400`}
                            >
                                Generate ({selectedDecades.length})
                            </button>
                         </div>
                    </div>
                )}
                                ...
                                {selectedDecades.map((decade) => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            transition={{ type: 'spring', stiffness: 100, damping: 20, delay: originalIndex * 0.15 }}
                                        >
                                            <PolaroidCard 
