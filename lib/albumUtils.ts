...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    // 2. Draw the title and credits
    ctx.fillStyle = '#333';
    ctx.textAlign = 'center';

    ctx.font = `bold 100px 'Caveat', cursive`;
    ctx.fillText('Generated with Time Travel', canvasWidth / 2, 150);
    ...
    const numImages = imagesWithDecades.length;
    const cols = numImages > 4 ? 3 : 2;
    const rows = Math.ceil(numImages / cols);

    const grid = { cols, rows, padding: 100 };
    const contentTopMargin = 350; // Space for the header and credits
    const contentHeight = canvasHeight - contentTopMargin;
    const cellWidth = (canvasWidth - grid.padding * (cols + 1)) / cols;
    const cellHeight = (contentHeight - grid.padding * (rows + 1)) / rows;
